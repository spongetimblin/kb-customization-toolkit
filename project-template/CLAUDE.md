...
3. **Prompt the user about a current-state snapshot** — check the date of the latest version folder or `current-state` folder. If more than one day has passed since the last session, ask: **"It's been more than a day since the last session. Do you want to create a `current-state` snapshot before we start? This captures any changes made in KnowledgeOwl since we last worked."** If the user confirms, walk them through copying all 12 code files from KnowledgeOwl into a new `YYYY.MM.DD-current-state` folder, and also create placeholder copies of any `full-html-snapshot-*.html` files found in the most recent version folder. Once the folder is populated, run `chmod -R a-w [current-state-folder]/` to make it read-only. Then proceed. The html snapshot placeholders in current-state are carry-forward markers only — the user should paste fresh HTML into the copies in the next version folder (which is writable), not into current-state.
...
A `YYYY.MM.DD-current-state` folder contains the 12 code files copied from KnowledgeOwl's Customize > Style (HTML & CSS) sections (the core snapshot), plus placeholder copies of any `full-html-snapshot-*.html` files found in the most recent version folder. The html snapshot files are not pulled from KnowledgeOwl — the user captures them from the browser — but always include placeholders for each one so the user knows to refresh them with current HTML.
